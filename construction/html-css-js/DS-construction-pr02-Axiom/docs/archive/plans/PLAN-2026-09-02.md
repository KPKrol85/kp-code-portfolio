...
- No item is currently prioritized; Phases 1 through 6 and both optional improvements (`O-01`, `O-02`) are complete.
...
- [x] **PH1-02 — Align the cache policy with the built output paths**
  ...
  - [x] give the production bundles a caching rule instead of inheriting `no-cache` from `/*` (at the time the fixed `style.min.css` and `script.min.js`; those URLs were later superseded by the content-addressed names from `O-01`)
...
- [x] **O-01 — Content-hashed filenames for production bundles**
  - [x] hash the final minified bytes of the fixed intermediates with SHA-256 truncated to 16 hexadecimal characters and rename them to `style.<hash>.min.css` and `script.<hash>.min.js` at the deployment root
  - [x] make `dist/build-manifest.json` the per-build source of truth for the final bundle filenames
  - [x] rewrite production HTML and the production service-worker precache from that manifest
  - [x] generate exact `Cache-Control: public, max-age=31536000, immutable` rules for those two URLs into `dist/_headers`, leaving `/css/*`, `/js/*`, and `js/theme-init.js` on finite revalidated caching
  - [x] make JavaScript bundle emission deterministic so identical sources produce identical filenames
  - **Value:** removes the trade-off behind `PH1-02` by making long-lived `immutable` caching safe for the production CSS and JavaScript bundles
  - **Scope boundary:** optional and broader than `PH1-02`, which is solvable with header rules alone; limited to the two production bundles — images, fonts, `manifest.webmanifest`, and `js/theme-init.js` keep their existing URLs
  - **Completion condition:** two clean builds from identical sources produce identical hashed bundle filenames, `build-manifest.json`, `dist/sw.js`, and `dist/_headers`; production HTML and the production service worker reference only those filenames; no unversioned URL is served `immutable`; and `npm run qa:references` passes both on a checkout without `dist/` and against the generated release
  - **Source:** `daily-AUDIT.md` — Content-hashed filenames for production bundles
