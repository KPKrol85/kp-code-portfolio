...
function validateJsonLdPayload(payload, filePath, blockNumber) {
  ...
  try {
    ...
    throw new Error(
      `${formatFilePath(filePath)} block ${blockNumber}: invalid JSON (${error.message})`,
    );
  }
  ...
  if (Array.isArray(parsed)) {
    if (parsed.length === 0) {
      throw new Error(
        `${formatFilePath(filePath)} block ${blockNumber}: JSON-LD array must not be empty`,
      );
    }
    ...
    parsed.forEach((item, index) => {
      validateJsonLdObject(
        item,
        `${formatFilePath(filePath)} block ${blockNumber}, item ${index + 1}`,
      );
    });
  ...
  validateJsonLdObject(
    parsed,
    `${formatFilePath(filePath)} block ${blockNumber}`,
  );
}
...
function validateJsonLd() {
  ...
  console.log(
    `JSON-LD validation passed (${validatedBlocks} blocks validated).`,
  );
}
