...
| SVG icon registry | all pages (footer contact rows + social links); pages with the scroll-up button; `index.html` about bullets, testimonials, and booking form | `[data-icon]`; optional `data-icon-class`, `data-icon-title`; `data-icon-ready` guard | `js/icons.js` | `initIcons()` | Renders registered SVG icons into placeholder hosts; icons are decorative unless `data-icon-title` is set. Footer hosts are `.site-footer__contact-icon` and the icon-only `.site-button--social` links; other hosts are `.scroll-btn__icon`, `.about__bullet-icon`, `.testimonials__rating-icon`, and `.booking-form__select-icon`. |
| Theme switcher | all pages with header | `[data-testid="site-header__theme-toggle"]` or `.site-header__theme-toggle`; `.theme-switch` SVG (`__core`, `__rays`, `__cutout`); root `html[data-theme]`, `html[data-theme-ready]` | `js/modules/theme.js` | `initThemeSwitcher()` | Persists theme in `localStorage` key `theme`; updates `aria-pressed`. `data-theme-ready` is set after the first paint so the sun/moon transition never runs on load. |
...
| Deferred footer map | all pages with the footer | `.site-footer__map`; `iframe[data-map-src]`; `[data-map-load]` | `js/modules/footer.js` | `initFooterMap()` | Reveals the loader button and, on its first click, moves `data-map-src` into the iframe `src`; the embed stays unloaded until then and receives focus once loaded. |
...
| Lightbox (gallery + dish thumbs) | pages with lightbox root and clickable media | `.site-lightbox` / `#lb`; `#lb-avif`; `#lb-webp`; `.site-lightbox__image` / `#lb-img`; `.site-lightbox__close`; `.site-lightbox__overlay`; `.gallery__item`; `.dish__thumb`; `[data-lightbox-mode]` scope on `.gallery__grid` / `.menu__grid`; optional `.site-lightbox__counter`, `.site-lightbox__nav-button--prev` / `--next` | `js/modules/lightbox.js` | `initLightbox()` | Opens from `.gallery__item` / `.dish__thumb`; adds keyboard, swipe, nav, zoom/fullscreen behaviors. The nearest `[data-lightbox-mode]` scope selects the session: `gallery` browses the visible group with counter and prev/next, `single` isolates one image and hides both; the resolved mode is mirrored on the dialog. |
